...
const Scale168Assessment: React.FC = () => {
  ...
  const saveAnswer = async (questionId: number, optionId: number) => {
    ...
    try {
      ...
      // 增加静默保存，不打扰用户
      await axios.post(getApiUrl(`/scale168/answers`), {
        scaleId: questionId,
        optionId
      }, {
        headers: {
  ...
  const getDimensionProgress = (category: string, dimension: Dimension) => {
    ...
    return {
      ...
      percent: dimensionQuestions.length ? Math.round((answeredCount / dimensionQuestions.length) * 100) : 0
    };
  };
        ...
        <StyledContent>
          ...
          <StyledCard
            title={
              <Space>
                ...
                <Progress
                  percent={((currentPage + 1) / Math.ceil(questions.length / questionsPerPage)) * 100}
                  format={() => `第 ${currentPage + 1}/${Math.ceil(questions.length / questionsPerPage)} 页`}
                  size="small"
                  style={{ width: 120 }}
          ...
          >
            {currentPageQuestions.map((question, index) => (
              <QuestionCard key={question.id} style={{ marginBottom: index < currentPageQuestions.length - 1 ? 16 : 0 }}>
            ...
            <Row justify="start" style={{ marginTop: '24px' }}>
              <Space>
                <Button 
                  ...
                  disabled={currentPage === 0 && currentDimension === '看' && currentCategory === categories[0].type}
                >
                  上一页
                </Button>
                <Button 
