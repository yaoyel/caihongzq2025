     ...
     WITH user_answers AS (
      ...
      final_scores AS (
        SELECT 
          ...
          -- 计算学术发展得分（四舍五入）
          ROUND(((COALESCE(ss.lexue_score, 0)  + COALESCE(ss.shanxue_score, 0) )  * 25 * 0.5) + (COALESCE(ss.academic_development_score, 0) / 100 * 25 * 0.5), 0) as academic_development_raw,
          -- 计算职业发展得分（四舍五入）
          ROUND((COALESCE(ss.lexue_score, 0) + COALESCE(ss.shanxue_score, 0) - (COALESCE(ds.tiaozhan_deduction, 0) + COALESCE(ds.yanxue_deduction, 0))) * 25 * 0.5 + COALESCE(ss.career_development_score, 0) / 100 * 25 * 0.5, 0) as career_development_raw,
          -- 计算行业前景得分（四舍五入）
          ROUND((COALESCE(ss.lexue_score, 0) + COALESCE(ss.shanxue_score, 0) - (COALESCE(ds.tiaozhan_deduction, 0) + COALESCE(ds.yanxue_deduction, 0))) * 25 * 0.5 + COALESCE(ss.growth_potential_score, 0) / 100 * 25 * 0.5, 0) as industry_prospects_raw,
          -- 计算成长潜力得分（四舍五入）
          ROUND((COALESCE(ss.career_development_score, 0) + COALESCE(ss.growth_potential_score, 0)) / 2 / 100 * 25 * 0.5 + COALESCE(ss.industry_prospects_score, 0) / 100 * 25 * 0.5, 0) as growth_potential_raw
      ...
      SELECT 
        ...
        -- 发展潜力得分（四舍五入）
        ROUND(CAST(fs.academic_development_raw + fs.career_development_raw + fs.industry_prospects_raw + fs.growth_potential_raw AS NUMERIC), 0)::NUMERIC as "developmentPotential",
        -- 机会得分（四舍五入）
        ROUND(CAST((fs.academic_development_raw + fs.career_development_raw + fs.industry_prospects_raw + fs.growth_potential_raw) / 2 + fs.base_score * 100 / 2 AS NUMERIC), 0)::NUMERIC as "opportunityScore",
        
        -- 各项得分（四舍五入）
        ROUND(CAST(fs.academic_development_raw AS NUMERIC), 0)::NUMERIC as "academicDevelopmentScore",
        ROUND(CAST(fs.career_development_raw AS NUMERIC), 0)::NUMERIC as "careerDevelopmentScore",
        ROUND(CAST(fs.industry_prospects_raw AS NUMERIC), 0)::NUMERIC as "industryProspectsScore",
        ROUND(CAST(fs.growth_potential_raw AS NUMERIC), 0)::NUMERIC as "growthPotentialScore"
