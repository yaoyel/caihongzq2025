...
export class MajorScoreService {
  ...
  async calculateMajorScores(userId: string): Promise<MajorScore[]> {
    ...
    const result = await this.majorDetailRepository.query(`
      ...
      study_scores AS (
        SELECT 
          ...
          ROUND(
            ...
            NULLIF(CAST(SUM(CASE WHEN type = 'lexue' THEN total_possible_score ELSE 0 END) AS NUMERIC), 0),
            2
          )::NUMERIC as lexue_score,
          ROUND(
            CAST(SUM(CASE WHEN type = 'shanxue' THEN weighted_score ELSE 0 END) AS NUMERIC) /
            NULLIF(CAST(SUM(CASE WHEN type = 'shanxue' THEN total_possible_score ELSE 0 END) AS NUMERIC), 0),
            2
      ...
      deduction_scores AS (
        SELECT 
          ts.major_code,
          MAX(CASE WHEN ts.type = 'tiaozhan' AND ts.type_score > 0 THEN 
            CASE 
              WHEN ts.potential_conversion_value = 'medium' THEN ts.type_ratio * 0.5
              WHEN ts.potential_conversion_value = 'low' THEN ts.type_ratio
              ELSE 0 
            END 
          ELSE 0 END) as tiaozhan_deduction,
          MAX(CASE WHEN ts.type = 'yanxue' AND ts.type_score > 0 THEN 
            CASE 
              WHEN ts.potential_conversion_value = 'medium' THEN ts.type_ratio * 0.5
              WHEN ts.potential_conversion_value = 'low' THEN ts.type_ratio
              ELSE 0 
            END 
          ELSE 0 END) as yanxue_deduction
        FROM type_scores ts
  ...
  async calculateSimpleMajorScores(userId: string): Promise<{ majorCode: string; majorName: string; eduLevel: string; score: number }[]> {
    const result = await this.majorDetailRepository.query(`
      WITH user_answers AS (
        SELECT 
          s.id as scale_id,
