...
export class MajorRedisService {
  ...
  async getMajorScores(
    ...
    try {
      ...
      if (batch) {
        ...
        // 如果没有指定批次，查找所有匹配的键
        const keys = await this.redisClient.keys(`${redisKey}_*`);
        
        // 获取所有批次的数据
        const allData = await Promise.all(
          keys.map(async key => {
            const data = await this.redisClient.zRange(key, start, end, { REV: true });
        ...
        return allData
          .flat()
          .sort((a: MajorHistoryScore, b: MajorHistoryScore) => {
   ...
   */
  private get2024Score(historyScore: string | null): number {
    try {
  ...
  async findMatchingMajors(
    ...
    try {
      ...
      results.forEach((result, index) => {
        ...
        const [subjectType, selection] = currentPattern.split(':');
  ...
  private calculateMatchLevel(
    ...
    const isOrCondition = patternSecond === '化学或生物';

    if (isOrCondition) {
      patternSubjects.push('化学', '生物');
    } else {
